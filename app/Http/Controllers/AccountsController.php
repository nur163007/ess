...
class AccountsController extends Controller
{
    ...
    public function accountStatement(Request $request)
    {
        ...
        if($data['account_id'] == 0){
            ...
            $lims_account_data = Account::get();

            if ($data['type'] == '0' || $data['type'] == '2') {
                $credit_list = Payment::whereNotNull('payments.sale_id')
                    ->whereDate('payments.created_at', '>=', $data['start_date'])
                    ->whereDate('payments.created_at', '<=', $data['end_date'])
                    ->select('payments.payment_reference as reference_no', 'payments.sale_id', 'payments.amount', 'payments.created_at','payments.paying_method','accounts.name as account_name')
                    ->join('accounts','payments.account_id', '=','accounts.id')
                    ->get();
                $recieved_money_transfer_list = MoneyTransfer::where('to_account_id', '!=','from_account_id')
                    ->whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'to_account_id',
                        DB::raw("(SELECT CONCAT('Received from ',accounts.account_no,'-',accounts.name) FROM accounts WHERE accounts.id = from_account_id) as ref_account"),
                        'amount', 'created_at')
                    ->get();
                $purchase_return_list = ReturnPurchase::whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'grand_total as amount', 'created_at')
                    ->get();
            }
            if ($data['type'] == '0' || $data['type'] == '1') {
                $debit_list = Payment::whereNotNull('payments.purchase_id')
                    ->whereDate('payments.created_at', '>=', $data['start_date'])
                    ->whereDate('payments.created_at', '<=', $data['end_date'])
                    ->select('payments.payment_reference as reference_no', 'payments.purchase_id', 'payments.amount', 'payments.created_at','payments.paying_method','accounts.name as account_name')
                    ->join('accounts','payments.account_id', '=','accounts.id')
                    ->get();
                $expense_list = Expense::whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'amount', 'created_at')
                    ->get();
                $return_list = Returns::whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'grand_total as amount', 'created_at')
                    ->get();
                $payroll_list = Payroll::whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'amount', 'created_at')
                    ->get();
                $sent_money_transfer_list = MoneyTransfer::where('from_account_id', '!=','to_account_id')
                    ->whereDate('created_at', '>=', $data['start_date'])
                    ->whereDate('created_at', '<=', $data['end_date'])
                    ->select('reference_no', 'to_account_id',
                        DB::raw("(SELECT CONCAT('Transferred to ',accounts.account_no,'-',accounts.name) FROM accounts WHERE accounts.id = to_account_id) as ref_account"),
                        'amount', 'created_at')
                    ->get();
            }
        ...
        $balance = 0;
        return view('backend.account.account_statement', compact('lims_account_data', 'all_transaction_list', 'balance','accountId'));
    }
