...
class TransferController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('transfers-index')) {
            ...
            $lims_from_warehouse_list = Warehouse::select('name', 'id')->where('main', true)->get();
            $lims_warehouse_list = Warehouse::select('name', 'id')->where('is_active', true)->get();
            return view('backend.transfer.index',compact('starting_date', 'ending_date', 'from_warehouse_id', 'to_warehouse_id', 'all_permission', 'lims_warehouse_list','lims_from_warehouse_list'));
        }
        else
    ...
    public function transferData(Request $request)
    {
        ...
        if(!empty($transfers))
        {
            foreach ($transfers as $key=>$transfer)
            {
                ...
                $nestedData['lc_no'] = $transfer->lc_no;
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('transfers-add')){
            $lims_warehouse_list = Warehouse::where('main', false)->get();
            $lims_agency_list = Agency::where('is_active', true)->get();
            $lims_lcno_list = Purchase::where('lc_no','!=', null)->get();
//            dd($lims_lcno_list);
            return view('backend.transfer.create', compact('lims_warehouse_list','lims_agency_list','lims_lcno_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }

    public function getProduct($id,$lcno)
    {
        $lims_product_warehouse_data = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
            ...
            ['product_purchases.lc_no', $lcno],
            ['product_warehouse.warehouse_id', $id],
            ['product_purchases.remaining', '>', 0]
        ])
        ...
        $lims_product_with_variant_warehouse_data = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
            ...
            ['product_warehouse.warehouse_id', $id],
            ['product_purchases.remaining', '>', 0]
        ])->whereNotNull('product_warehouse.variant_id')->select('product_purchases.*')->get();
...
//            }
            ...
            if($data['status'] != 2) {
                ...
                $product_purchase = ProductPurchase::where([
                    ['lc_no', $lcno],
                    ['product_id', $id]
                ])->first();

                $product_purchase->transfer_qty +=$qty[$i];
                $product_purchase->remaining -=$qty[$i];
...
//                    $lims_product_warehouse_data->product_batch_id = $product_transfer['product_batch_id'];
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('transfers-edit')){
            $lims_from_warehouse_list = Warehouse::where('main',true)->get();
            $lims_warehouse_list = Warehouse::where('main',false)->get();
            $lims_agency_list = Agency::where('is_active',true)->get();
...
//                }
    ...
    public function getWarehouse($lcno){
        $data = Purchase::where("lc_no", $lcno)->get();
//        dd($data);
//        $warehouseid = [];
        foreach ($data as $list){
            $warehouseid = Warehouse::where('id',$list->warehouse_id)->pluck('name','id');
        }
        return json_encode($warehouseid);
    }
}
