...
class ReportController extends Controller
{
    ...
    public function warehouseReport(Request $request)
    {
        ...
        if ($warehouse_id == 0){
            $lims_purchase_data = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            $lims_sale_data = Sale::with('customer')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            $lims_quotation_data = Quotation::with('customer')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            $lims_return_data = Returns::with('customer', 'biller')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            $lims_expense_data = Expense::with('expenseCategory')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            $lims_summary_data = Warehouse::orderBy('created_at', 'desc')->get();
            $lims_specific_warehouse = Warehouse::first();
        }
        else{
            ...
            $lims_summary_data = Warehouse::where('id', $warehouse_id)->orderBy('created_at', 'desc')->get();
            $lims_specific_warehouse = Warehouse::where('id',$warehouse_id)->first();
        }
        ...
        $purchase_total = 0;
        $purchase_qty = 0;
        $purchase_total_return = 0;
        $purchase_qty_return = 0;
        $transfer_total_out = 0;
        $transfer_qty_out = 0;
        $transfer_total_in=0;
        $transfer_qty_in=0;
        $sales_total=0;
        $sales_qty=0;
        $sales_total_return=0;
        $sales_qty_return=0;
        $total_product_qty=0;
        $total_sales_qty=0;
        $stock_in_hand=0;
        ...
        if ($warehouse_id == 0){
            foreach ($lims_summary_data as $key => $sum_data){
                $wid = $sum_data->id;

                $all_summary_data[$key] = Warehouse::with('purchase','purchasereturn','transferout','transferin','sale','returnsale')->where('id','=',$wid)->first();
            ...
            //purchases table data
                $lims_product_summary_purchase= DB::select("SELECT sum(`total_cost`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from purchases where warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
                foreach ($lims_product_summary_purchase as $key => $pur){
                    $purchase_total +=$pur->purchase_cost1;
                    $purchase_qty +=$pur->purchase_qty1;
                }

            //purchases return table data
            $lims_product_summary_return_purchase= DB::select("SELECT sum(`total_cost`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from return_purchases where warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
            foreach ($lims_product_summary_return_purchase as $key => $return_pur){
                $purchase_total_return +=$return_pur->purchase_cost1;
                $purchase_qty_return +=$return_pur->purchase_qty1;
            }

            //transfer table data out
            $lims_product_summary_transfer_out= DB::select("SELECT sum(`total_cost`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from transfers where from_warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
            foreach ($lims_product_summary_transfer_out as $key => $transfer_out){
                $transfer_total_out +=$transfer_out->purchase_cost1;
                $transfer_qty_out +=$transfer_out->purchase_qty1;
            }

            //trnasfer table data in
            $lims_product_summary_transfer_in= DB::select("SELECT sum(`total_cost`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from transfers where to_warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
            foreach ($lims_product_summary_transfer_in as $key => $transfer_in){
                $transfer_total_in +=$transfer_in->purchase_cost1;
                $transfer_qty_in +=$transfer_in->purchase_qty1;
            }

            //sales table data
            $lims_product_summary_sales= DB::select("SELECT sum(`total_price`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from sales where warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
            foreach ($lims_product_summary_sales as $key => $sales){
                $sales_total +=$sales->purchase_cost1;
                $sales_qty +=$sales->purchase_qty1;
            }

            //sales table return data
            $lims_product_summary_sales_return= DB::select("SELECT sum(`total_price`) as `purchase_cost1`,
                    sum(`total_qty`) as `purchase_qty1` from returns where warehouse_id = $warehouse_id and created_at between '$start_date' and '$end_date'");
            foreach ($lims_product_summary_sales_return as $key => $sales_return){
                $sales_total_return +=$sales_return->purchase_cost1;
                $sales_qty_return +=$sales_return->purchase_qty1;
            }

            $total_product_qty =($purchase_qty-$transfer_qty_out+$transfer_qty_in+$sales_qty_return-$purchase_qty_return);
            $total_sales_qty = $sales_qty-$sales_qty_return;
            $stock_in_hand = $total_product_qty-$total_sales_qty;
        ...
        return view('backend.report.warehouse_report', compact('warehouse_id', 'start_date', 'end_date', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_sale_data', 'lims_product_sale_data', 'lims_warehouse_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data', 'lims_expense_data','total_product_qty','total_sales_qty','stock_in_hand','all_summary_data','lims_summary_data','lims_specific_warehouse'));
    }
...
//        $lims_product_transfer_data = [];
        ...
        $lims_purchase_data = Purchase::with('warehouse','shipping')->where('lc_no', $lcno)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_transfer_data = Transfer::with('fromWarehouse', 'toWarehouse','shipping')->where('lc_no', $lcno)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();

//dd($lims_transfer_data);
        foreach ($lims_purchase_data as $key => $purchase) {
            $lims_product_purchase_data[$key] = ProductPurchase::where('purchase_id', $purchase->id)->get();
        }
//        foreach ($lims_transfer_data as $key => $transfer) {
//            $lims_product_transfer_data[$key] = ProductTransfer::where('transfer_id', $transfer->id)->get();
//        }
        $lims_product_transfer_data = ProductTransfer::select('product_transfer.*','shippings.shipping_type','transfers.status','w1.name as fromWarehouse','w2.name as toWarehouse','product_purchases.qty as total_qty')
                                                        ->leftJoin('shippings', 'product_transfer.transfer_id', '=', 'shippings.transfer_id')
                                                        ->leftJoin('transfers', 'product_transfer.transfer_id', '=', 'transfers.id')
                                                        ->leftJoin('warehouses as w1', 'transfers.from_warehouse_id', '=', 'w1.id')
                                                        ->leftJoin('warehouses as w2', 'transfers.to_warehouse_id', '=', 'w2.id')
                                                        ->leftJoin('product_purchases', 'product_transfer.lc_no', '=', 'product_purchases.lc_no')
                                                        ->where('product_transfer.lc_no',$lcno)->get();
//        dd($lims_product_transfer_data);

        $lims_lcno_list = Purchase::where('lc_no','!=', null)->get();
        return view('backend.report.lc_report', compact('lcno', 'start_date', 'end_date', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_transfer_data', 'lims_product_transfer_data', 'lims_lcno_list') );
    }
    ...
    public function customerReport(Request $request)
    {
        ...
        $lims_payment_data = DB::table('payments')
                           ...
                           ->whereDate('payments.created_at', '>=' , $start_date)
                           ->whereDate('payments.created_at', '<=' , $end_date)
                           ->select('payments.*', 'sales.reference_no as sale_reference','accounts.name as account_name')
        ...
        return view('backend.report.customer_report', compact('lims_sale_data','customer_id', 'start_date', 'end_date', 'lims_product_sale_data', 'lims_payment_data', 'lims_customer_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data','customer_info'));
    }
    ...
    public function supplierReport(Request $request)
    {
        ...
        return view('backend.report.supplier_report', compact('lims_purchase_data', 'lims_product_purchase_data', 'lims_payment_data', 'supplier_id', 'start_date', 'end_date', 'lims_supplier_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data','supplier_info'));
    }
    ...
    public function customerDueReportByDate(Request $request)
    {
        ...
        $lims_customer_list = Customer::where('is_active', true)->get();
        return view('backend.report.due_report', compact('lims_sale_data', 'start_date', 'end_date','customer_id','customer_info','lims_customer_list'));
    }
    ...
    public function supplierDueReportByDate(Request $request)
    {
        ...
        $q = Purchase::where('payment_status', 1)
            ...
            ->whereDate('created_at', '<=' , $end_date);
        if($supplier_id)
            $q = $q->where('supplier_id', $supplier_id);
        $lims_purchase_data = $q->get();
        $supplier_info = Supplier::where('id',$supplier_id)->first();
        $lims_supplier_list = Supplier::where('is_active', true)->get();
        return view('backend.report.supplier_due_report', compact('lims_purchase_data', 'start_date', 'end_date','supplier_id','supplier_info','lims_supplier_list'));
    }
}
