...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('agency-index')){
            $permission = Permission::firstOrCreate(['name' => 'agency-index']);
            if(!$role->hasPermissionTo('agency-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('agency-index');

        if($request->has('agency-add')){
            $permission = Permission::firstOrCreate(['name' => 'agency-add']);
            if(!$role->hasPermissionTo('agency-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('agency-add');

        if($request->has('agency-edit')){
            $permission = Permission::firstOrCreate(['name' => 'agency-edit']);
            if(!$role->hasPermissionTo('agency-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('agency-edit');

        if($request->has('agency-delete')){
            $permission = Permission::firstOrCreate(['name' => 'agency-delete']);
            if(!$role->hasPermissionTo('agency-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('agency-delete');
        ...
        if($request->has('lc-report')){
            $permission = Permission::firstOrCreate(['name' => 'lc-report']);
            if(!$role->hasPermissionTo('lc-report')){
                $role->givePermissionTo($permission);
            }
         }
        else
            $role->revokePermissionTo('lc-report');
        ...
        if($request->has('card')) {
            $permission = Permission::firstOrCreate(['name' => 'card']);
            if(!$role->hasPermissionTo('card')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('card');
