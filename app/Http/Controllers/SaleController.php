...
//                $nestedData['deposit_data'] = Deposit::select("account_id")->where("customer_id", $sale->customer->id)->first();
//                if ($nestedData['deposit_data']){
//                    $nestedData['deposit_data'] = $nestedData['deposit_data'];
//                }
//                $nestedData['grand_total'] = number_format($sale->grand_total, 2);
                $nestedData['grand_total'] = number_format($sale->total_price - $sale->order_discount, 2);
...
//                $nestedData['due'] = number_format($sale->grand_total - $returned_amount - $sale->paid_amount, 2);
                $nestedData['due'] = number_format(($sale->total_price - $sale->order_discount) - $returned_amount - $sale->paid_amount, 2);
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            $lims_account_list = Account::where('is_active', true)->get();
            return view('backend.sale.create',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','lims_agency_list','lims_account_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $mail_data['shipping_agency'] = $lims_sale_data->shipping_agency;
        $mail_data['shipping_type'] = $lims_sale_data->shipping_type;
        $mail_data['shipping_vehicle_no'] = $lims_sale_data->shipping_vehicle_no;
...
//            $lims_account_data = Account::where('is_default', true)->first();
            ...
            $lims_payment_data->payment_date = date("Y-m-d h:i:s");
...
//        shipping agency shipping cost vehicle no
