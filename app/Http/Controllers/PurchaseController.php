...
class PurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('purchases-index')) {
            ...
            $lims_warehouse_list = Warehouse::where('main', true)->get();
    ...
    public function purchaseData(Request $request)
    {
        ...
        if(!empty($purchases))
        {
            foreach ($purchases as $key=>$purchase)
            {
                ...
                $nestedData['lc_no'] = $purchase->lc_no;
                ...
                $nestedData['grand_total'] = number_format($purchase->total_cost - $purchase->order_discount , 2);
//                $nestedData['grand_total'] = number_format($purchase->grand_total , 2);
                ...
                $nestedData['purchase_due'] = number_format(($purchase->total_cost - $purchase->order_discount)- $returned_amount  - $purchase->paid_amount, 2);
//                $nestedData['shipping_due'] = number_format($purchase->grand_total - (($purchase->total_cost - $purchase->order_discount)- $returned_amount  - $purchase->paid_amount) - $purchase->paid_amount , 2);
...
//                        </li>';
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('purchases-add')){
            ...
            return view('backend.purchase.create', compact('lims_supplier_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_agency_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request,[
            'lc_no' => 'required|min:5|max:60|unique:purchases'
        ]);
        if ($document) {
            $v = Validator::make(
                [
...
//                $lims_product_warehouse_data->product_batch_id = $product_purchase['product_batch_id'];
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('purchases-edit')){
            ...
            $lims_product_purchase_data = ProductPurchase::where('purchase_id', $id)->get();
            $lims_agency_list = Agency::where('is_active', true)->get();
            $lims_shipping_data = Shipping::with('agency')->where('purchase_id',$id)->get();
            return view('backend.purchase.edit', compact('lims_warehouse_list', 'lims_supplier_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_tax_list', 'lims_purchase_data', 'lims_product_purchase_data','lims_agency_list','lims_shipping_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
...
//        dd($data);
        ...
        $this->validate($request, [
            'lc_no' => [
                'max:255',
                Rule::unique('purchases')->ignore($id)->where(function ($query) {
                    return $query->where('lc_no','!=',null);
                }),
            ],
        ]);
        if ($document) {
...
//                $lims_product_warehouse_data->product_batch_id = $product_purchase['product_batch_id'];
            ...
            $product_purchase['remaining'] = $recieved[$key];
            $product_purchase['purchase_unit_id'] = $lims_purchase_unit_data->id;
...
//                }
    ...
    public function getAllAgency(){
        ...
        $lims_agency_list = DB::select("SELECT id,company_name as name from agencies where is_active =1");
        return json_encode($lims_agency_list);
    }
}
