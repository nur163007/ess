...
<section class="forms">
    ...
    <ul class="nav nav-tabs ml-4 mt-3" role="tablist">
      ...
      <li class="nav-item">
       <a class="nav-link" href="#warehouse-summary" role="tab" data-toggle="tab">{{trans('file.Summary')}}</a>
      </li>
    </ul>
...
//                    $total_due = 0;
                    ...
                    @foreach($lims_purchase_data as $key=>$purchase)
                            ...
                            $total_purchase_amount +=$purchase->grand_total;
                            $total_paid +=$purchase->paid_amount;
                            ?>
                    @endforeach
                            <?php
                            $total_due = $total_purchase_amount - $total_paid;
                            ?>
                        <tr>
                            <th></th>
                            <th>Total:</th>
...
{{--                            <td>{{$quotation->reference_no}}</td>--}}
                            ...
                            <td>
                                ...
                                ?>
                                @if($unit)
                                    {{$product->name.' ('.$product_quotation_data->qty.' '.$unit->unit_name.')'}}
                                @else
...
{{--                            <td>{{$return->reference_no}}</td>--}}
                            ...
                            <td>
                                ...
                                ?>
                                @if($unit)
                                    {{$product->name.' ('.$product_return_data->qty.' '.$unit->unit_name.')'}}
                                @else
...
{{--                            <th></th>--}}
        ...
        <div role="tabpanel" class="tab-pane fade" id="warehouse-summary">
            <div class="table-responsive mb-4">
                <table id="summary-table" class="table table-hover">
                    <thead>
                    <tr>
                        <th class="not-exported-summary"></th>
                        <th>{{trans('file.Warehouse')}}</th>
                        <th>{{trans('file.product_qty')}} </th>
                        <th>{{trans('file.Sales Qty')}}</th>
                        <th>{{trans('file.stock_in_hand')}}</th>
                    </tr>
                    </thead>
                    <tbody>
                    @if($warehouse_id == 0)
                    @foreach($all_summary_data as $key=>$summary)
                        <?php
                        $purchase_data = $all_summary_data[$key]['purchase'];
                        $purchase_return_data = $all_summary_data[$key]['purchasereturn'];
                        $purchase_transOut_data = $all_summary_data[$key]['transferout'];
                        $purchase_transIn_data = $all_summary_data[$key]['transferin'];
                        $total_sale_data = $all_summary_data[$key]['sale'];
                        $total_return_data = $all_summary_data[$key]['returnsale'];
                        $all_purchase_qty = 0;
                        $all_purchase_qty_return = 0;
                        $all_transfer_qty_out = 0;
                        $all_transfer_qty_in=0;
                        $all_sales_qty=0;
                        $all_sales_qty_return=0;
                        $all_product_qty=0;
                        $all_sales_quantity=0;
                        $all_stock_in_hand=0;
                        ?>
                        @if($purchase_data)
                            @foreach($purchase_data as $key=>$pur_summary)
                                <?php
                                $all_purchase_qty += $pur_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        @if($purchase_return_data)
                            @foreach($purchase_return_data as $key=>$purRet_summary)
                                <?php
                                $all_purchase_qty_return += $purRet_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        @if($purchase_transOut_data)
                            @foreach($purchase_transOut_data as $key=>$transOut_summary)
                                <?php
                                $all_transfer_qty_out += $transOut_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        @if($purchase_transIn_data)
                            @foreach($purchase_transIn_data as $key=>$transIn_summary)
                                <?php
                                $all_transfer_qty_in += $transIn_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        @if($total_sale_data)
                            @foreach($total_sale_data as $key=>$sale_summary)
                                <?php
                                $all_sales_qty += $sale_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        @if($total_return_data)
                            @foreach($total_return_data as $key=>$sale_return_summary)
                                <?php
                                $all_sales_qty_return += $sale_return_summary->total_qty;
                                ?>
                            @endforeach
                        @endif
                        <?php
                        $all_product_qty = $all_purchase_qty-$all_transfer_qty_out+$all_transfer_qty_in+$all_sales_qty_return-$all_purchase_qty_return;
                        $all_sales_quantity = $all_sales_qty-$all_sales_qty_return;
                        $all_stock_in_hand = $all_product_qty-$all_sales_quantity;

//                        echo $purchase_transOut_data;
                        ?>

                        <tr>
                            <td>{{$key}}</td>
                            <td>{{ $summary->name }}</td>
                            <td> {{ $all_product_qty }} </td>
                            <td> {{ $all_sales_quantity }} </td>
                            <td> {{ $all_stock_in_hand }} </td>
                        </tr>
                    @endforeach
                    @else
                        @foreach($lims_summary_data as $key=>$specific_data)
                        <tr>
                            <td>{{$key}}</td>
                            <td>{{ $specific_data->name}}</td>
                            <td> {{ $total_product_qty }} </td>
                            <td> {{ $total_sales_qty }} </td>
                            <td> {{ $stock_in_hand }} </td>
                        </tr>
                        @endforeach
                    @endif
...
<script type="text/javascript">
    ...
    var war_id = <?php echo json_encode($warehouse_id)?>;

    if(war_id == 0){
        warehouse_name = 'All Stockyard';
        warehouse_phone = 'N/A';
        warehouse_email= 'N/A';
    }else{
        warehouse_name = <?php echo json_encode($lims_specific_warehouse->name)?>;
        warehouse_phone = <?php echo json_encode($lims_specific_warehouse->phone) ?>;
        warehouse_email = <?php echo json_encode($lims_specific_warehouse->email) ?>;
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard sale report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
                action: function(e, dt, button, config) {
                    datatable_sum_sale(dt, true);
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard purchase report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard quotation report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard retrun report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard expense report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
    ...
    $('#summary-table').DataTable( {
        ...
        buttons: [
            {
                extend: 'pdf',
                exportOptions: {
                    columns: ':visible:Not(.not-exported-summary)',
                    rows: ':visible'
                },
                action: function(e, dt, button, config) {
                    datatable_sum_summary(dt, true);
                    $.fn.dataTable.ext.buttons.pdfHtml5.action.call(this, e, dt, button, config);
                    datatable_sum_summary(dt, false);
                },
                footer:true
            },
            {
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Stockyard summary report</b></h1><br>' +
                        '<h3 class="text-left">Warehouse name  : '+warehouse_name+'</h3><br>' +
                        '<h3 class="text-left">Warehouse phone : '+warehouse_phone+'</h3><br>' +
                        '<h3 class="text-left">Warehouse email : '+warehouse_email+'</h3><hr><br>'
                },
        ...
        drawCallback: function () {
            var api = this.api();
            datatable_sum_summary(api, false);
        }
    } );

    function datatable_sum_summary(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            var rows = dt_selector.rows( '.selected' ).indexes();
