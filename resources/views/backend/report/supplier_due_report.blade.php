...
{{--                        <td>{{$purchase_data->reference_no}}</td>--}}
                        ...
                        <td>{{number_format((float)( $purchase_data->total_cost - $purchase_data->order_discount ), 2, '.', '')}}</td>
                        <td>{{number_format((float)$returned_amount, 2, '.', '')}}</td>
                        @if($purchase_data->paid_amount)
                        <td>{{number_format((float)(($purchase_data->grand_total - (($purchase_data->total_cost - $purchase_data->order_discount)- $returned_amount  - $purchase_data->paid_amount))-$purchase_data->shipping_cost), 2, '.', '')}}</td>
                        @else
                        <td>0.00</td>
                        @endif
                        <td>{{number_format((float)(($purchase_data->total_cost - $purchase_data->order_discount)- $returned_amount  - $purchase_data->paid_amount), 2, '.', '')}}</td>
                    </tr>
                    @endif
                @endforeach
            </tbody>
...
<script type="text/javascript">
    ...
    var supplier_name = <?php echo json_encode($supplier_info->name) ?>;
    var supplier_phone = <?php echo json_encode($supplier_info->phone_number) ?>;
    var supplier_email = <?php echo json_encode($supplier_info->email) ?>;
    if(supplier_phone != null){
        supplier_phone = <?php echo json_encode($supplier_info->phone_number) ?>;
    }
    else{
        supplier_phone = 'N/A';
    }
    ...
    if(supplier_email != null){
        supplier_email = <?php echo json_encode($supplier_info->email) ?>;
    }
    else{
        supplier_email = 'N/A';
    }
        ...
        buttons: [
            ...
            },
            {
                ...
                repeatingHead: {
                    ...
                    logoPosition: 'center',
                    logoStyle: '',
                    title: '<br><h1 class="text-left"><b>Supplier due report</b></h1><br>' +
                        '<h3 class="text-left">Supplier name  : '+supplier_name+'</h3><br>' +
                        '<h3 class="text-left">Supplier phone : '+supplier_phone+'</h3><br>' +
                        '<h3 class="text-left">Supplier email : '+supplier_email+'</h3><hr><br>'
                },
                action: function(e, dt, button, config) {
                    datatable_sum(dt, true);
